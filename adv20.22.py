...
# for zi, z in enumerate(tiles_dict[current_tile]):
#     if len(borders_dict[z]) == 1:
#         print(zi)
puzzle_lb = [[3]]       # change!
puzzle = [[current_tile]]
current_line = 0
        ...
        if puzzle_lb[i][j] == 1:  # flip horizontal
            for iii in range(len(tiles[puzzle[i][j]])):
                for jjj in range(len(tiles[puzzle[i][j]][0])):
                    new_tiles[puzzle[i][j]][iii][jjj] = tiles[puzzle[i][j]][10 - iii - 1][jjj]
        elif puzzle_lb[i][j] == 2:  # flip vertical
            for iii in range(len(tiles[puzzle[i][j]])):
                for jjj in range(len(tiles[puzzle[i][j]][0])):
                    new_tiles[puzzle[i][j]][iii][jjj] = tiles[puzzle[i][j]][iii][10 - jjj - 1]
...
puplus = [[] for i in range(11*len(puzzle))]
for ii, puzz in enumerate(puzzle):
    for jj, ti in enumerate(puzz):
        for i in range(10):
            puplus[i + ii*11].extend(list(new_tiles[ti][i])+[' '])
        ...
        for j in range(len(pu[0]) - 19):
            ...
            for elem in snake[snake_counter]:
                if pu[i + elem[0]][j + elem[1]] == 0:
                    ...
                    found.append((i + elem[0], j + elem[1]))
...
for snake_counter in range(4, 8):
    found_all = []
    for i in range(len(pu) - 19):
        for j in range(len(pu[0]) - 2):
            found = []
            for elem in snake[snake_counter]:
                if pu[i + elem[0]][j + elem[1]] == 0:
                    found = []
